...
class ImageDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100, load_in_tmp=True):

        self.detections_path = detections_path
        self.gri_feat= os.path.join(self.detections_path, 'gri_feat.h5')
        self.gri_mask= os.path.join(self.detections_path, 'gri_mask.h5')
        self.reg_feat= os.path.join(self.detections_path, 'reg_feat.h5')
        self.reg_mask= os.path.join(self.detections_path, 'reg_mask.h5')
        self.pre_box = os.path.join(self.detections_path, 'pre_box.h5')

        self.g_f = h5py.File(self.gri_feat, 'r')
        self.g_m = h5py.File(self.gri_mask, 'r')
        self.r_f = h5py.File(self.reg_feat, 'r')
        self.r_m = h5py.File(self.reg_mask, 'r')
        self.box = h5py.File(self.pre_box, 'r')
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        outputs['gri_feat'] = self.g_f['%d' % image_id][()]
        outputs['gri_mask'] = self.g_m['%d' % image_id][()]
        outputs['reg_feat'] = self.r_f['%d' % image_id][()]
        outputs['reg_mask'] = self.r_m['%d' % image_id][()]
        outputs['box'] = self.box['%d' % image_id][()]

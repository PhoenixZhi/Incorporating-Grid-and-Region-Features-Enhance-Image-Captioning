...
class BeamSearch(object):
    ...
    def _expand_visual(self, visual: utils.TensorOrSequence, cur_beam_size: int, selected_beam: torch.Tensor):
        if isinstance(visual, torch.Tensor):
            ...
            visual = torch.gather(visual_exp, 1, selected_beam_exp.long()).view(visual_red_shape)
        else:
            new_visual = {}
            for name,im in visual.items():
                ...
                selected_beam_exp = selected_beam.view(selected_beam_red_size).expand(selected_beam_exp_size)
                new_im = torch.gather(visual_exp, 1, selected_beam_exp.long()).view(visual_red_shape)
                new_visual[name]=new_im
            visual = new_visual
        return visual
    ...
    def apply(self, visual: utils.TensorOrSequence, out_size=1, return_probs=False, **kwargs):
        ...
        outputs = torch.gather(outputs, 1, sort_idxs.expand(self.b_s, self.beam_size, self.max_len).long())
        log_probs = torch.cat(self.log_probs, -1)
        log_probs = torch.gather(log_probs, 1, sort_idxs.expand(self.b_s, self.beam_size, self.max_len).long())
        if return_probs:
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence, outputs, return_probs, **kwargs):
        ...
        self.seq_mask = torch.gather(self.seq_mask, 1, selected_beam.unsqueeze(-1).long())
        outputs = list(torch.gather(o, 1, selected_beam.unsqueeze(-1).long()) for o in outputs)
        ...
        this_word_logprob = torch.gather(word_logprob, 1,
                                         selected_beam.unsqueeze(-1).expand(self.b_s, self.beam_size,
                                                                            word_logprob.shape[-1]).long())
        this_word_logprob = torch.gather(this_word_logprob, 2, selected_words.unsqueeze(-1).long())
        self.log_probs = list(
            torch.gather(o, 1, selected_beam.unsqueeze(-1).expand(self.b_s, self.beam_size, 1).long()) for o in self.log_probs)
